...
const fetchIntrastatResults = async ({ queryKey }) => {
  ...
  return sbClientWithToken.get(`${Endpoint.INTRASTRASTS}/results?${searchParams.toString()}`).then((res) => res.data)
}
...
const fetchIntrastatErrors = async ({ queryKey }) => {
  ...
  return sbClientWithToken.get(`${Endpoint.INTRASTRASTS}/errors?${searchParams.toString()}`).then((res) => res.data)
}
...
const postInstrastatValidate = async (data) => {
  ...
  const sbClientWithToken = await getSbClientWithToken()
  return sbClientWithToken.post(`${Endpoint.INTRASTRASTS}/validate`, { month, year })
}
...
const fetchIntrastatWithoutCode = async () => {
  const sbClientWithToken = await getSbClientWithToken()
  return sbClientWithToken.get(`${Endpoint.INTRASTRASTS}/items-without-code`).then((res) => res.data)
}
...
const fetchIntrastatMissingTaxNumbers = async () => {
  const sbClientWithToken = await getSbClientWithToken()
  return sbClientWithToken.get(`${Endpoint.INTRASTRASTS}/missing-tax-numbers`).then((res) => res.data)
}
